...
export default function RezerwacjeBilardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(`/api/reservations/bilard?date=${encodeURIComponent(day)}`);
        const json = (await res.json().catch(() => null)) as AvailabilityResponse | null;

        if (!alive) return;

        // fallback jak coś nie tak
        if (!res.ok || !json || (json as any).ok !== true) {
          setCfg({ pricePerHour: 50, resourcesCount: 3, startHour: 16, endHour: 22 });
          return;
        }

        const ok = json as Extract<AvailabilityResponse, { ok: true }>;
        const resourcesCount = Array.isArray(ok.resources) && ok.resources.length ? ok.resources.length : 3;

        // godziny bierzemy z tego co API zwraca w slots (pierwszy i ostatni slot)
        ...
        setCfg({
          pricePerHour: Number(ok.pricePerHour ?? 50),
          resourcesCount,
          startHour,
          endHour,
        });
      } catch {
        if (alive) setCfg({ pricePerHour: 50, resourcesCount: 3, startHour: 16, endHour: 22 });
      }
    }
  ...
  const form = useForm<BilliardsRequest>({
    resolver: zodResolver(billiardsRequestSchema),
    defaultValues: {
      type: "bilard",
      date: day,
      startHour: 0,
  ...
  useEffect(() => {
    ...
    // ✅ backward-compatible pola (żeby schema się zgadzała)
    ...
    // ✅ NOWE: segmenty wrzucamy do values (schema tego nie zna, więc as any)
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (parsed?.error === "NO_AVAILABILITY" || res.status === 409) {
          setConflictMessage("Wybrane stoły nie są już dostępne. Odświeżyliśmy dostępność — wybierz inny termin lub stół.");
  ...
  return (
    <div className="grid gap-6">
      ...
      {step === 2 ? (
        <form className="grid gap-6" onSubmit={form.handleSubmit(onSubmit)} aria-label="Formularz rezerwacji bilardu">
          <Card>
            <CardHeader>
              ...
              <div className="rounded-xl border p-4">
                ...
                {grid.segments.length ? (
                  <div className="mt-3 grid gap-3">
                    <div>
                      <p className="font-semibold">Bilard</p>
                    ...
                    <div className="grid gap-1.5">
                      {grid.segments.map((s) => (
                        ...
                        >
                          <span className="font-medium">Stół {s.resource}: <span className="font-normal text-muted-foreground">{formatHHMMFromFloat(s.startHour)}–{formatHHMMFromFloat(s.endHour + 1)}</span></span>
                  ...
                  <div className="mt-2">
                    <p className="font-medium">
                      Bilard • {day} • {startLabel}–{endLabel} • Wybrane stoły: {grid.resources.length}
                    </p>
