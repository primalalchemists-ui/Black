...
export async function POST(req: Request) {
  ...
  if (useSegments) {
    ...
    try {
      ...
      for (const resourceId of sortedResourceIds) {
        const lk = `kregle|${data.date}|resource:${resourceId}`;
        await lockClient.query(`SELECT pg_advisory_lock(hashtext($1))`, [lk]);
        acquiredLockKeys.push(lk);
      }
    } catch (lockErr) {
      console.error("[kregle] advisory lock error:", lockErr);
      if (lockClient) {
  ...
  return NextResponse.json({
    ...
    if (msg.includes("zarezerwowany w tym czasie") || msg.startsWith("Kolizja")) {
      return NextResponse.json(
        { error: "NO_AVAILABILITY", message: "Wybrany tor jest już zajęty w tym terminie. Odśwież stronę i wybierz inny czas." },
        { status: 409 }
      )
    }
    console.error("[kregle] POST nieoczekiwany błąd:", msg)
    return NextResponse.json(
