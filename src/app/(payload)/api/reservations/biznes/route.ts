...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error("[biznes] advisory lock error:", lockErr)
      if (lockClient) {
    ...
    try {
      ...
      reservationNumber = await getNextReservationNumber(payload, "W")
      ...
      if (requiresPayment) {
        ...
        console.log(`[biznes] P24_REGISTER_STARTED reservationNumber=${reservationNumber} groupId=${groupId} amount=${amountGrosze}`)
        try {
          ...
          console.log(`[biznes] P24_REGISTER_OK reservationNumber=${reservationNumber} groupId=${groupId}`)
          ...
          try {
            ...
            console.log(`[biznes] PAYMENT_RECORD_CREATED reservationNumber=${reservationNumber} groupId=${groupId} amount=${amountGrosze / 100}`)
            ...
            console.error(`[biznes] PAYMENT_RECORD_CREATE_FAILED reservationNumber=${reservationNumber} groupId=${groupId}:`, payErr?.message)
          }
          ...
          if (replaceSessionId) {
              ...
              for (const oldDoc of freshResResult.docs as any[]) {
                if (oldDoc.paymentStatus === "pending") {
                  try {
                    ...
                    console.error(`[biznes] REPLACE_EXPIRE_DOC_FAILED id=${oldDoc.id}:`, perErr?.message)
                  }
                }
              }
            ...
            if (expireResult === null) {
              console.error(`[biznes] CRITICAL REPLACE_EXPIRE_LOCK_UNAVAILABLE replaceSessionId=${replaceSessionId} oldId=${oldReservationId} newId=${reservationDoc.id} newRN=${reservationNumber} — callback aktywny podczas wygaszenia`)
              replaceConflict = "lock_unavailable"
            } else if ("conflict" in expireResult) {
              console.error(`[biznes] CRITICAL REPLACE_PAID_DURING_SWAP replaceSessionId=${replaceSessionId} oldId=${oldReservationId} conflict=${expireResult.conflict} newId=${reservationDoc.id} newRN=${reservationNumber} — stara sesja opłacona podczas swapa, manual review needed`)
              replaceConflict = expireResult.conflict
            } else {
              console.log(`[biznes] REPLACE_OLD_EXPIRED replaceSessionId=${replaceSessionId} oldId=${oldReservationId} count=${expireResult.expiredCount} newId=${reservationDoc.id} newRN=${reservationNumber}`)
            }
          }
          ...
          if (replaceConflict) {
            ...
            try {
              ...
              console.error(`[biznes] CRITICAL REPLACE_CONFLICT_EXPIRE_FAILED newId=${reservationDoc.id} — slot NIE zwolniony, fallback expiresAt=${reservationDoc.expiresAt ?? "brak"}:`, e?.message)
            }
            if (createdPaymentId !== null) {
              try {
                ...
                console.error(`[biznes] CRITICAL REPLACE_CONFLICT_PAYMENT_FAILED pmtId=${createdPaymentId}:`, e?.message)
              }
            }
            if (resExpired) {
              console.error(`[biznes] CRITICAL REPLACE_CONFLICT_NEW_ABANDONED newId=${reservationDoc.id} newRN=${reservationNumber} newPmtId=${createdPaymentId ?? "none"} pmtFailed=${pmtMarkedFailed} p24SessionId=${groupId} replaceConflict=${replaceConflict} — slot zwolniony natychmiast`)
            } else {
              console.error(`[biznes] CRITICAL REPLACE_CONFLICT_SLOT_NOT_FREED newId=${reservationDoc.id} newRN=${reservationNumber} newPmtId=${createdPaymentId ?? "none"} p24SessionId=${groupId} replaceConflict=${replaceConflict} — slot NIE zwolniony, fallback expiresAt=${reservationDoc.expiresAt ?? "brak"}`)
            }
            return NextResponse.json({
          ...
          console.error(`[biznes] P24_REGISTER_FAILED reservationNumber=${reservationNumber} groupId=${groupId}:`, p24Err?.message ?? p24Err)
          await payload.update({
            collection: "reservations",
            id: reservationDoc.id,
            overrideAccess: true,
            data: { status: "cancelled", paymentStatus: "failed", cancellationReason: "payment_failed" } as any,
          }).catch((e) => console.error("[biznes] rollback update failed:", e))
